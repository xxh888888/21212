...
-- 悬浮球拖拽和菜单
当地的 balldraging，ballStartPos，ballStartTouch=you，you
当地的 menuOpen=

Inputbegin:Connect（功能，gpe）
如果全球教育伙伴关系然后 返回 结束
如果输入. UserInputType====Enu. UserInputType1输入. UserInputType====Enu. UserInputType
拖球=真正的
...
球
如果全球教育伙伴关系然后 返回 结束
如果输入。UserInputType=====Enu。UserInputType1输入。UserInputType=====Enu
如果拖球和球赛
如果（Vector2.new（input.Position.X，input.Position.Y）-ballStartTouch）5然后
menuOpen=不菜谱
...
Heartbeat:Connect（功能（））
如果拖拽和 triggerDragStartPos you
当地的 mp=UserInputService:GetMouseLocation（）
当地的 delta=Vector2
当地的 newPos=triggerDragStartPos+delta
...
当地的 mp=UserInputService:GetMouseLocation（）
当地的 d=Vector2
ball.Position = UDim2.fromOffset(math.clamp((ballStartPos + d).X,0, gui.AbsoluteSize.X -50), math.clamp((ballStartPos + d).Y,0好的。当然50))
如果菜谱然后菜单=yodo you.60, 0, 0) 结束
结束
结束)
...
玩家
    ...
    task.wait(0.1)
如果启用然后 you you=char:FindFirstChild"Humanoid"）；如果 H you WalkSpeed=currentSpeed
如果攀登然后你想爬连接：
如果启用然后 disableFly（）；task.wait（0.1）；enableFly（）结束
如果旋转然后你怎么看 spinConnection:Disconnect（）enableSpin（）
如果启用然后
-- 重新应用骨骼变换
结束
结束)
Players.PlayerRemoving：连接功能（plr）
        task.wait(0.2)
        saveOriginalBones(char)
        updateAllBones(char)
        createBoneWidgets(char)
如果 esphights[plr]you mayoto sphights[plr]：Destroy（）；esphights[plr]=
结束)

-- 初始化
为_，T bioipairs（{speedToggle，climbToggle，flyToggle，spinToggle，espoTggle，aimbotToggle，dodgeToggle，orbitToggle，boneTgoggle}）
更新切换（T，you）
...
setSpeed(50); setDodgeRadius(50); setOrbitDistance(100triggerDragging =50); setOrbitSpeed(15)
setOrbitRadius(20triggerDragStartPos =5); setAimbotRadius(10triggerDragStartTouch =200); setAimTriggerSize(200)
updateBoneSliders()
