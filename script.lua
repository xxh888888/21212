...
local aimbotCard = createCard(scrollingFrame, 6, 195)
...
local dodgeCard = createCard(scrollingFrame, 7, 100)
...
local orbitCard = createCard(scrollingFrame, 8, 175)
...
local teleportCard = createCard(scrollingFrame, 9, 42)
...
local function setFlyPanelSize(v)
    ...
    if flyControlPanel then
        flyControlPanel.Size = UDim2.new(0, v, 0, v * 1.8)  -- 增加高度以容纳速度滑块
        local speedSliderFrame = flyControlPanel:FindFirstChild("SpeedSliderFrame")
        if speedSliderFrame then
            speedSliderFrame.Size = UDim2.new(0.8, 0, 0, v * 0.18)
            speedSliderFrame.Position = UDim2.new(0.1, 0, 0.85, 0)
        end
    end
end
...
local function createTeleportWindow()
    ...
    local playerAddedConnection = Players.PlayerAdded:Connect(function() updatePlayerList() end)
...
local function disableTeleport()
    stopFollow()
    if teleportWindow then teleportWindow:Destroy(); teleportWindow = nil end
    teleportEnabled = false
end
...
local function createFlyControlPanel()
    ...
    flyControlPanel.Size = UDim2.new(0, psize, 0, psize * 1.8)  -- 增加高度
        ...
        if pd and psp and pst then
            ...
            if mp then
                ...
                flyControlPanel.Position = UDim2.fromOffset(
                    ...
                    math.clamp((psp + d).Y, 0, gui.AbsoluteSize.Y - psize * 1.8)
                )
            end
        end
    end)
    ...
    local function cb(text, pos, color)
        ...
        b.Size = UDim2.new(0, psize*0.25, 0, psize*0.25)
        b.Position = pos
    ...
    -- 飞行速度滑块（在面板底部）
    local speedSliderFrame = Instance.new("Frame", flyControlPanel)
    speedSliderFrame.Name = "SpeedSliderFrame"
    speedSliderFrame.Size = UDim2.new(0.8, 0, 0, psize * 0.18)
    speedSliderFrame.Position = UDim2.new(0.1, 0, 0.85, 0)
    ...
    speedSliderBtn.Position = UDim2.new(ratio, -6, 0.5, -6)
    speedSliderBtn.BackgroundColor3 = Color3.new(1, 1, 1)
    speedSliderBtn.AutoButtonColor = false
    speedSliderBtn.Text = ""
    speedSliderBtn.ZIndex = 10003
    
    local speedValueLabel = Instance.new("TextLabel", speedSliderFrame)
    speedValueLabel.Size = UDim2.new(1, 0, 0.3, 0)
    speedValueLabel.Position = UDim2.new(0, 0, 0.7, 0)
    speedValueLabel.BackgroundTransparency = 1
    speedValueLabel.Text = tostring(flySpeed)
    speedValueLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    ...
    speedSliderBg.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            draggingSpeed = true
            local ratio = math.clamp((input.Position.X - speedSliderBg.AbsolutePosition.X) / speedSliderBg.AbsoluteSize.X, 0, 1)
            local newSpeed = math.floor(10 + ratio * 490)
            setFlySpeed(newSpeed)
            speedLabel.Text = "飞行速度: " .. newSpeed
            speedValueLabel.Text = tostring(newSpeed)
            ratio = (newSpeed - 10) / 490
            speedSliderFill.Size = UDim2.new(ratio, 0, 1, 0)
            speedSliderBtn.Position = UDim2.new(ratio, -6, 0.5, -6)
        end
    end)
    ...
    local function sm(dir)
        ...
        elseif dir == "Backward" then moveDirection = -cf
        end
...
local function enableAimbot()
    ...
    aimbotConnection = RunService.Heartbeat:Connect(function()
        ...
        if ch then
            cam.CFrame = cam.CFrame:Lerp(CFrame.lookAt(cam.CFrame.Position, ch.Position), aimStrength)
        end
    end)
end
...
aimTriggerBtn.InputEnded:Connect(function(input, gpe)
    if gpe then return end
    if aimTriggerMoving then triggerDragging = false; triggerDragStartPos = nil; triggerDragStartTouch = nil end
end)
...
-- 初始化
for _, t in ipairs({speedToggle, climbToggle, flyToggle, spinToggle, espToggle, aimbotToggle, dodgeToggle, orbitToggle, teleportToggle}) do
